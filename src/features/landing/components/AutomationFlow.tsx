...
export function AutomationFlow({ title, description, steps, delay = 0 }: AutomationFlowProps) {
  return (
    <motion.div
      ...
      className="p-6 rounded-xl border border-border bg-card backdrop-blur-sm"
    >
        ...
        {steps.map((step, idx) => (
          <div key={idx} className="flex items-center gap-3 flex-shrink-0">
            <motion.div
              ...
              className="px-4 py-2 rounded-lg border border-border bg-muted min-w-[140px] text-center"
              style={{
